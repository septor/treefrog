...
module.exports = {
    ...
    async execute(message, args) {
        if (!canPostInChannel(this.name, message.channel.id)) {
            ...
            return message.channel.send(`\`${this.name}\` can only be used in the following channels: ${allowedChannels}`);
        }
        ...
        message.channel.send('Please provide the codes you have checked, separated by commas:')
            .then(() => {
                message.channel.awaitMessages({ filter, max: 1, time: 60000, errors: ['time'] })
                    .then(collected => {
                        const checkedCodes = collected.first().content.split('\n').map(code => code.trim());
                        const updatepoint = config.updatepoint;

                        axios.post(updatepoint, qs.stringify({
                            action: 'checked',
                        ...
                        .then(response => {
                            ...
                            if (updatedCodes.length > 0) {
                                message.channel.send(`Updated ${updatedCodes.length} codes to 'needs_verified' based on your input.`);
                            } else {
                    ...
                    .catch(() => {
                        message.author.send(`You didn't provide any codes in the alloted time. Please initiate \`!checked\` again when you have more time.`);
                    });
            })
            .catch(error => {
